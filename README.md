...
- **자재 LOT** — 입고 자재 LOT·잔량 등록, **CODE128 바코드**·썸네일·**150×100mm 라벨 인쇄**, 재고 반영(IN)
- **LOT 이력·자재 투입** — LOT 생명주기 및 투입 이력 조회

### 생산 실행·품질

- **통합 생산 운영** — 계획·지시·LOT·실적을 한 화면에서 탭 전환하며 운영
- **당일 생산 손익** (`/production-daily-pl`) — 품목별 예상 손익·추이·매출 구성
- **생산 원가 기준** (`/production-cost-basis`) — 입률·제품원가·판매단가 기준 관리
...
### 당일 생산 손익·원가 기준 (V1.3.4)

- **당일 생산 손익** (`/production-daily-pl`) — 품목별 예상 손익, 일별 추이, 매출 구성 차트(흑자 손익 분리 / 적자 빗금 손실)
- **생산 원가 기준** (`/production-cost-basis`) — 입률·제품원가·판매단가 등 산정 기준
- 입고단가·품목 바코드·사용자 서명/부서 등 기준정보·ERP 결재 보강
- 상세: [RELEASE-V1.3.4.md](./RELEASE-V1.3.4.md)

### LOT 라벨·스캔 조회 (V1.3.3)

- **생산·자재 LOT 라벨** — 150×100mm (ZD421 203dpi), Zebra ZPL + 브라우저 폴백, 하단 **(주)하나테크**
- **LOT 스캔 조회** (`/lot-scan`) — 생산 LOT + **자재 LOT** 통합 조회 (바코드/LOT번호)
- 상세: [RELEASE-V1.3.3.md](./RELEASE-V1.3.3.md)
...
| 문서 | 대상 | 내용 |
|------|------|------|
| **[RELEASE-V1.3.4.md](./RELEASE-V1.3.4.md)** | 개발·운영 | **V1.3.4** 당일 생산 손익·원가 기준·입고단가·결재 서명 |
| **[RELEASE-V1.3.3.md](./RELEASE-V1.3.3.md)** | 개발·운영 | **V1.3.3** LOT 라벨 인쇄·스캔 조회·현장 다국어 |
...
- **V1.3.4** 기준: **당일 생산 손익**, **생산 원가 기준**, 입고단가·품목 바코드·ERP 결재 서명. **V1.3.3** — LOT 라벨·스캔·현장 다국어. **V1.3.2** — 자재 LOT 바코드·자동 배정. **V1.3.0** — ERP·급여·목록 UI 등.
...
**당일 생산 손익 · 원가 기준 · 입고단가 · 결재 서명**

- **당일 생산 손익**: 품목별 손익·추이·매출 구성 차트 (흑자 손익 분리 / 적자 빗금 손실)
- **생산 원가 기준**: 입률·제품원가·판매단가 관리, EBOM 자재비 연동
- 입고단가·재고 단가 유틸, 품목 바코드 보강
- 사용자 부서·직책·서명·사업팀·주소, 지출결의/연차 결재 서명
- 상세: [RELEASE-V1.3.4.md](./RELEASE-V1.3.4.md)

### V1.3.3 (2026-07-31)

**LOT 라벨 인쇄 · 스캔 조회 · 현장 다국어**

- **생산·자재 LOT 150×100mm 라벨**: Zebra ZD421 ZPL + 브라우저 PNG 폴백, 203dpi 이진화, (주)하나테크 푸터
- 공유 모듈: `printBarcode.ts`, `labelPrintProfile.ts`, `zplPrint.ts`
- 바코드 API `view=label` (`CODE128_LABEL`) — 생산·자재 LOT
- **LOT 스캔 조회**: 자재 LOT lookup API·모달 (`GET /api/material-lots/lookup`)
- **현장 입력** 한/영/베트남어 i18n (`src/client/i18n/`)
- 상세: [RELEASE-V1.3.3.md](./RELEASE-V1.3.3.md)
